...
select ename, sal
from emp
where sal not between 2000 and 3000   -- not 은 키워드 앞에
;
...
where deptno in (20, 30)  -- wrong answer _(where deptno between 20 and 30)
order by ename desc
;
...
from emp
where (sal>=2000 and sal<=3000) and (deptno=20 or deptno=30)  -- (where sal between 2000 and 3000 and deptno in(20,30))
...
from emp
where comm is not null and comm>0  -- is no null 써야함 *
...
from emp
where ( job in ('CLERK', 'SALESMAN') ) and ( sal not in(1600, 950, 1300) )  --(where job in('CLERE', 'SALESMAN')and not sal in(1600,950,1300))      
;
