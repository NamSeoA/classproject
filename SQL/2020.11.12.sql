...
-- 사원 테이블과 유사한 구조의 사원번호, 사원이름, 급여 3개의 칼럼으로 구성된 EMP01 테이블을 생성해봅시다.
desc emp;
...
-- create table  ~ as ~  : 테이블을 복사, 다만 제약조건은 복사되지 않는다.
-- create table 명령어 다음에 컬럼을 일일이 정의하는 대신 
-- as절을 추가하여 emp 테이블과 동일한 내용과 구조를 갖는
-- emp02 테이블을 생성해봅시다
create table emp02
as
select * from emp;
...
desc emp02;
desc emp;  -- empno not null 제약이 있음
select * from emp02;
...
-- 서브쿼리문의 select문을 구성할 때
-- where절을 추가하여 원하는 조건을 제시하면
...
-- 기존 테이블의 컬럼 변경 : 새로 정의된 컬럼으로 교체 하는 것
alter table emp01
...
-- default : insert시에 데이터가 입력되지 않을 때 자동으로 등록되는 데이터 정의
drop table emp07;
create table emp07 (
    empno number(4) primary key,
    ename varchar2(10) not null,
