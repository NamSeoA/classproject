...
-- USING (inner, on~ 생략가능 -> 더 간결해짐)
select *
from emp join dept
-- on emp.deptno = dept.deptno
USING(deptno)                    -- 공통컬럼은 하나만 가지게 해준다. 공통컬럼을 구별할 필요가 없다.
;
...
-- Non Equi JOIN : 동등 비교가 아닌 비교연산이 가능하다
-- 급여 등급을 5개로 나누어 놓은 SALGRAD에서 정보를 얻어와서
-- 각 사원의 급여 등급을 지정해보자
-- 사원(EMP)테이블과 급여 등급(SALGRADE) 테이블을 조인하도록 합시다
-- 사원의 급여가 몇등급인지 살펴보자
...
-- 중첩질의        - WHERE 부속질의, 단일 열, 단순비교 -> 단일행, 집합 비교 -> 복합 행
...
-- 스칼라 부속질의 : select 절 프레젠테이션 영역에 쓸 수 있는 부속질의, 단일행, 단일열, 단일값의 결과만 나와야한다.
...
-- 인라인 뷰 : from 절 이후에 들어가는 서브 쿼리, 가상 테이블, rownum이 새롭게 생성
...
from orders
where saleprice <= (select avg(saleprice) from orders)         -- 평균주문금액
;
...
from orders o1
where saleprice >
(select avg(saleprice) from orders o2 where o2.custid=o1.custid) -- custid=1이면 1번 고객의 평균과 비교해서 큰 주문금액만 출력
;
